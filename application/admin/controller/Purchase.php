...
//        Db::insert("insert into cg_purchase(custom,jiesuanfangshi,pjlx,storage,storage,originarea)values('$custom','$jisuan','$pjlx','$storage','$texture','$orgin')");
    ...
    function single(){
        //$dat=Db::table('c_purchase')->join('stock_out','c_purchase.system_id=stock_out.id')->join('c_product_type','c_product_type.id =c_purchase.c_leibie_id')->where('c_purchase.static','=',0)->select();
        $dat=Db::table('c_purchase')->field('*,c_purchase.id')->join('stock_out','c_purchase.system_id=stock_out.id')->join('c_product_type','c_product_type.id =c_purchase.c_leibie_id')->select();
